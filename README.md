...
- Для сервиса Data Importer необходим файл credentials.json, который предоставляется тимлидом (Secrets на GitHub). 
- Содержимое файла установить как переменную окружения (см. раздел "Секреты").
...
Если у вас архитектура arm64 необходимо дополнительно прописать в переменные окружения ARCH=-arm64
...
default.yaml - файл конфигурации автоматического добавления дашборда в Grafana UI